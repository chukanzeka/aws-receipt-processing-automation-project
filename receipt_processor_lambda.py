...
SNS_ERROR_TOPIC_ARN = os.environ.get('SNS_ERROR_TOPIC_ARN')  # New environment variable
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"Error processing receipt: {error_message}")
        print(f"Full traceback: {error_traceback}")
        
        # Send SNS alert for the failure
        send_failure_alert(bucket, key, error_message, error_traceback, context)
...
def send_failure_alert(bucket, key, error_message, error_traceback, context):
    """Send SNS alert when receipt processing fails"""
    try:
        if not SNS_ERROR_TOPIC_ARN:
            print("SNS_ERROR_TOPIC_ARN not configured, skipping failure alert")
            return
        ...
        alert_data = {
            ...
            "error_type": "Receipt Processing Failure"
        }
        
        # Create a user-friendly subject
        subject = f"🚨 Receipt Processing Failed: {key or 'Unknown File'}"
        
        # Create detailed message
        message = f"""
Receipt processing has failed with the following details:

📁 File: s3://{bucket or 'Unknown'}/{key or 'Unknown'}
⏰ Timestamp: {alert_data['timestamp']}
🔧 Function: {alert_data['function_name']} (v{alert_data['function_version']})
🆔 Request ID: {alert_data['request_id']}
...
Please investigate and resolve the issue.
        ...
        sns.publish(
            ...
            MessageAttributes={
                'error_type': {
                    'DataType': 'String',
                    'StringValue': 'receipt_processing_failure'
                },
                's3_bucket': {
                    'DataType': 'String',
                    'StringValue': bucket or 'unknown'
                },
                'function_name': {
                    'DataType': 'String',
                    'StringValue': context.function_name if context else 'unknown'
                }
            }
        )
        
        print(f"Failure alert sent to SNS topic: {SNS_ERROR_TOPIC_ARN}")
        
    except Exception as sns_error:
        print(f"Failed to send SNS alert: {str(sns_error)}")
        # Don't re-raise this error as it would mask the original failure
...
def process_receipt_with_textract(bucket, key):
    ...
    try:
        ...
        raise Exception(f"Textract processing failed: {str(e)}")
